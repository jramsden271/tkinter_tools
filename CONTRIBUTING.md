...
- `main` is always in a working state. Don't commit directly to it.
- Create a branch per feature or fix: `<type>/<short-description>`, e.g. `feat/queue-cancel-all`, `fix/entry-dnd-crash`.
- Open a PR into `main` when the branch is ready. Merge via PR (squash or regular merge, your call) rather than pushing straight to `main`.
